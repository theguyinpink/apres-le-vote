...
export default async function PartyPage({
  ...
  const pageUrl = `${baseUrl}/partis/${party.slug}`;
  const pageTitle = `${party.name} 2027 : programme, mesures et effets | Après le vote`;
    ...
    "@graph": [
      {
        ...
        about: {
          ...
          ...(party.logo
            ? {
                logo: party.logo.startsWith("http")
                  ? party.logo
                  : `${baseUrl}${party.logo.startsWith("/") ? "" : "/"}${party.logo}`,
              }
            : {}),
        },
        creator: {
        ...
        citation: [
          party.sourceUrl,
          ...(party.extraSources?.map((source) => source.url) ?? []),
        ],
      },
      {
        "@type": "BreadcrumbList",
    ...
    <main className="party-page" style={style}>
      ...
      <section className="party-hero">
        ...
        <div className="party-hero-grid">
          ...
          <div
            className={`party-title-block ${party.slug === "rn" ? "party-title-block--rn" : ""}`}
          >
            <p>{party.family}</p>
      ...
      <div className="party-content section-shell">
          ...
          <div className="section-heading compact-heading program-heading">
            <div>
              <p className="section-index">
                01 — Le programme complet, en clair
              </p>

              <h2>
                Pas quatre promesses.
                <br />
                Tout le sommaire.
              </h2>
            </div>
            ...
            <p>
              Chaque chapitre ou dossier officiel est représenté. Ouvre une
              rubrique pour lire son idée centrale, tous ses sous-thèmes et
              retrouver le texte source — sans devoir traverser des centaines de
              pages.
            </p>
          </div>
        ...
        <section className="projection-section" id="projection">
          ...
          <div className="projection-method">
            ...
            <p>
              Majorité absolue stable, programme appliqué, pas de crise
              extérieure majeure, croissance et taux d’intérêt favorables.
              Population estimée à partir de la trajectoire centrale de l’Insee,
              ajustée seulement quand la politique migratoire du projet l’impose
              clairement.
            </p>
          </div>
          ...
          <p className="projection-sources">
            Repères :{" "}
            <a
              href="https://www.insee.fr/fr/statistiques/2107840"
              target="_blank"
              rel="noreferrer"
            >
              Insee — conjoncture 2026 ↗
            </a>
            <span> · </span>
            <a
            ...
            <span> · </span>
            <a
              href="https://www.insee.fr/fr/statistiques/2381470"
              target="_blank"
              rel="noreferrer"
            >
        ...
        <section className="votes-section" id="votes">
          <div className="section-heading compact-heading votes-heading">
            ...
            <p>
              {voteContext ? (
                <>
                  Référence parlementaire retenue :{" "}
                  <strong>{voteContext.representation}</strong>.{" "}
                  {voteContext.note}
                </>
              ) : (
                <>
                  Le groupe parlementaire lié à ce parti est aujourd’hui «{" "}
                  {party.assemblyGroup} ». Un groupe n’est pas exactement un
                  parti : le nom et sa composition sont donc précisés pour
                  chaque scrutin.
                </>
              )}
            </p>
          ...
          <div className="vote-method-note">
            ...
            <p>
              « Position dominante » signifie le choix le plus fréquent parmi
              les votes enregistrés. Quand un mouvement n’a pas de groupe
              propre, nous affichons uniquement les votes des députés dont
              l’appartenance au mouvement est documentée. Les absents ne sont
              pas comptés.
            </p>

            <div className="vote-key" aria-label="Légende des votes">
              <span>
                <i className="key-for" /> Pour
              </span>

              <span>
                <i className="key-against" /> Contre
              </span>

              <span>
                <i className="key-abstain" /> Abstention
              </span>
            </div>
          </div>

          <div className="vote-list">
            {landmarkVotes.some((vote) => vote.positions[party.slug]) ? (
              landmarkVotes.map((vote) => {
                ...
                const total =
                  position.for + position.against + position.abstain;
                  ...
                  <article className="vote-card" key={vote.id}>
                    <div className="vote-card-head">
                      ...
                      <p>
                        {vote.date}
                        <br />
                        <small>{vote.legislature}</small>
                      </p>
                    </div>

                    <p className="vote-plain">
                      <strong>En clair :</strong> {vote.plain}
                    </p>

                    <div className="party-vote-result">
                      <div>
                        <span>
                          {position.label ?? "Position dominante du groupe"}
                        </span>

                        <strong
                          className={`dominant dominant-${dominant.toLowerCase()}`}
                        >
                          {dominant}
                        </strong>
                      ...
                      <div className="vote-chart-wrap">
                        ...
                        >
                          {position.for > 0 && (
                            <span
                              className="vote-segment vote-for"
                              style={{ flexGrow: position.for }}
                            />
                          )}

                          {position.against > 0 && (
                            <span
                              className="vote-segment vote-against"
                              style={{ flexGrow: position.against }}
                            />
                          )}

                          {position.abstain > 0 && (
                            <span
                              className="vote-segment vote-abstain"
                              style={{ flexGrow: position.abstain }}
                            />
                          )}
                        </div>
                        ...
                        <div className="vote-counts">
                          <span>
                            <b>{position.for}</b> pour
                          </span>

                          <span>
                            <b>{position.against}</b> contre
                          </span>

                          <span>
                            <b>{position.abstain}</b> abst.
                          </span>

                          <span>
                            <b>{total}</b> votes affichés
                          </span>
                        </div>
                      </div>
                    </div>
      ...
      <section className="sources-section section-shell">
        ...
        <div className="sources-grid">
          ...
          <div>
            ...
            <a
              href="https://www.assemblee-nationale.fr/dyn/les-donnees-ouvertes"
              target="_blank"
              rel="noreferrer"
            >
              Assemblée nationale — données ouvertes ↗
            </a>
            ...
            <a
              ...
              rel="noreferrer"
            >
              Insee — comptes publics 2025 ↗
            </a>

            <a
              href="https://www.vie-publique.fr/questions-reponses/294788-quoi-sert-une-majorite-lassemblee-nationale"
              target="_blank"
              rel="noreferrer"
            >
              Vie publique — rôle de la majorité ↗
            </a>
          </div>

          <p>
            Cette fiche croise les documents disponibles au 17 août 2026 et,
            lorsque la correspondance parlementaire est suffisamment fiable, des
            scrutins publics de 2023 à 2025. Un vote renseigne sur une décision
            précise, pas sur toute l’idéologie d’un parti. Le scénario sera
